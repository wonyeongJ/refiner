...
ORACLE_TO_JAVA: dict[str, str] = {
    ...
    # 숫자 타입 (정밀도 있는 경우 BigDecimal, 없으면 Long/Integer)
    "NUMBER": "BigDecimal",
    "NUMERIC": "BigDecimal",
    "DECIMAL": "BigDecimal",
    "FLOAT": "Double",
...
# NUMBER(p, s) 에서 scale=0 이고 precision 작으면 Integer/Long 사용
NUMBER_INT_THRESHOLD = 10  # precision <= 10 이면 Long, 아니면 BigDecimal


def _get_java_type(oracle_type: str, precision: Optional[int] = None, scale: Optional[int] = None) -> str:
    """
    Oracle 타입 문자열을 Java 타입 문자열로 변환.
    NUMBER(p, 0) → Long/Integer 로 스마트 변환.
    """
    upper_type = oracle_type.strip().upper()

    # TIMESTAMP(n) 처리
    if upper_type.startswith("TIMESTAMP"):
        if "WITH TIME ZONE" in upper_type:
            return "OffsetDateTime"
        return "LocalDateTime"

    # NUMBER 정밀도 처리
    if upper_type in ("NUMBER", "NUMERIC", "DECIMAL"):
        if scale is not None and scale == 0:
            if precision is not None and precision <= 9:
                ...
                return "BigDecimal"
        elif scale is None and precision is not None and precision <= 9:
            return "Integer"
        return "BigDecimal"
...
def _parse_column_def(col_def: str) -> Optional[dict]:
    ...
    if timestamp_tz_match:
        ...
        return {
            ...
            "precision": None,
            "scale": None,
            "java_type": _get_java_type(oracle_type),
        }
    ...
    java_type = _get_java_type(oracle_type, precision, scale)
